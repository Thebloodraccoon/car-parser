...
async def get_car_by_id(car_id: str, car_crud: CarCRUD = Depends(get_car_crud)):
...
async def create_car(car: CarCreate, car_crud: CarCRUD = Depends(get_car_crud)):
    return await car_crud.create_car(car)


@router.put("/{car_id}", response_model=CarResponse)
async def update_car(
    car_id: str, car_update: CarUpdate, car_crud: CarCRUD = Depends(get_car_crud)
):
    return await car_crud.update_car(car_id, car_update)


@router.delete("/{car_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_car(car_id: str, car_crud: CarCRUD = Depends(get_car_crud)):
